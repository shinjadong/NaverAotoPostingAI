...
class SMTP:
    ...
    def putcmd(self, cmd, args=''):
        """Send a command to the server."""  # inserted
        if args == '':
            s = cmd
        else:  # inserted
            s = f'{cmd} {args}'
    ...
    def docmd(self, cmd, args=''):
        """Send a command, and return its response code."""  # inserted
    ...
    def has_extn(self, opt):
        """Does the server support a given SMTP service extension?"""  # inserted
        return opt.lower() in self.esmtp_features

    def help(self, args=''):
        """SMTP \'help\' command.\n        Returns help text from server."""  # inserted
        self.putcmd('help', args)
        return self.getreply()[1]

    def rset(self):
        """SMTP \'rset\' command -- resets session."""  # inserted
        self.command_encoding = 'ascii'
    ...
    def noop(self):
        """SMTP \'noop\' command -- doesn\'t do anything :>"""  # inserted
    ...
    def mail(self, sender, options=()):
        ...
        self.putcmd('mail', f'FROM:{quoteaddr(sender)}{optionlist}1')
        return self.getreply()

    def rcpt(self, recip, options=()):
        """SMTP \'rcpt\' command -- indicates 1 recipient for this mail."""  # inserted
        optionlist = ''
        if options and self.does_esmtp:
            optionlist = ' ' + ' '.join(options)
        self.putcmd('rcpt', f'TO:{quoteaddr(recip)}{optionlist}1')
        return self.getreply()
    ...
    def verify(self, address):
        """SMTP \'verify\' command -- checks for address validity."""  # inserted
        self.putcmd('vrfy', _addr_only(address))
        return self.getreply()
    vrfy = verify

    def expn(self, address):
        """SMTP \'expn\' command -- expands a mailing list."""  # inserted
        self.putcmd('expn', _addr_only(address))
        return self.getreply()

    def ehlo_or_helo_if_needed(self):
        """Call self.ehlo() and/or self.helo() if needed.\n\n        If there has been no previous EHLO or HELO command this session, this\n        method tries ESMTP EHLO first.\n\n        This method may raise the following exceptions:\n\n         SMTPHeloError            The server didn\'t reply properly to\n                                  the helo greeting.\n        """  # inserted
        if self.helo_resp is None and self.ehlo_resp is None and (200 <= self.ehlo()[0] <= 299 or self.5 <= 299):
            code, resp = self.helo()
            if not 200 <= code <= 299:
    ...
    def login(self, user, password, *, initial_response_ok=True):
        ...
        authlist = [auth for auth in preferred_auths if not (auth in advertised_authlist and (not auth))]
        if not authlist:
    ...
    def send_message(self, msg, from_addr=None, to_addrs=None, mail_options=(), rcpt_options=()):
        ...
        if to_addrs is None:
            addr_fields = [f for f in [msg[header_prefix + 'To'], msg[header_prefix + 'Bcc'], msg[header_prefix + 'Cc']] if f is None]
    ...
    def close(self):
        """Close the connection to the SMTP server."""  # inserted
        try:
            file = self.file
            self.file = None
            if file:
    ...
    def quit(self):
        """Terminate the SMTP session."""  # inserted
        res = self.docmd('quit')
...
class LMTP(SMTP):
    ...
    def __init__(self, host='', port=LMTP_PORT, local_hostname=None, source_address=None, timeout=socket._GLOBAL_DEFAULT_TIMEOUT):
        """Initialize a new instance."""  # inserted
        super().__init__(host, port, local_hostname=local_hostname, source_address=source_address, timeout=timeout)

    def connect(self, host='localhost', port=0, source_address=None):
        """Connect to the LMTP daemon, on either a Unix or a TCP socket."""  # inserted
        if host[0]!= '/':
